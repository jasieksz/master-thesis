...
because it has the most balanced distribution of number of profiles, between the selected preference restrictions.
...
Figure \ref{fig:40-40-examples-grid} (in the 2nd column, 1st and 2nd row).
...
it is clear that the choice of the radii distribution can significantly reinforce or subside
trends in the numbers of profiles with a selected property.
...
we observe significant increase in the number of profiles with the CR and VR properties,
respectively for the GCR/SUVR and the SUCR/GVR.
...
increasing the number of the VR or CR profiles.
...
we showed that the VCR domain can be significantly larger than the VR and CR ones combined.
...
A natural question that arises from the previous experiments, 
...
If we fail to find a solution, we use a number $5$ as a result, which we interpret as 5 or more agent deletions required. 
...
We use the similar ideas and concepts as in previous experiments to explain the results.
...
First category is the radius candidate/voter model used to generate the profiles, 
and second one is based on the transformation result parameter, i.e, the CR, VR and CRVR types.
...
% To represent deleted agent type, i.e., candidate (the CD method) or voter (the VD method), we use different colors. 
% Furthermore, we distinguish the position candidate/voter models on the x-axis. 
...
Each subplot consists of 4 violins, which are similar to a box and whisker plot. 
A violin plot shows the distribution of quantitative data, and uses kernel density estimation for the visualization. 
In our case, violins are additionally split in half, i.e., the left part (light color) shows a number of candidates deleted,
and the right part (dark color) a number of voters deleted.
...
e.g., top-left subplot is a visualization of results for the GCR/GVR radius model and the CR property.
Furthermore, position candidate/voter models are represented on x-axis.
}
...
We notice that they are somewhat symmetrical. 
For example, for the GCP/GVP position model, to obtain the CR profile it is preferable to delete voters.
However, if we look at the VR case, it is better to delete candidates.
When saying preferred or better, we mean it requires fewer agent removals.
Another interesting observation in these plots, are the violins for the UCP/GVP and the GCP/UVP models.
They are virtually swapped between the CR and the VR cases.
To explain this, we go back to the respective histograms from Experiment 2 in Figure \ref{fig:ex42-20C20V-bars-RGG}.
For the UCP/GVP model, the majority of profiles are VR (not counting the TVCR ones).
...
There is nothing unexpected in results for the CRVR property (Figure \ref{fig:ex43-vln-RGCR-GVR-PCRVR} and Figure \ref{fig:ex43-vln-RLUCR-LUVR-PCRVR}).
...
Let us discuss the second part of the experiment,
that is results for the CVD method,
...
Only difference is conceptual. Previously violins were split to distinguish between removed agent type,
i.e, left half (light color) represented candidates and right half (dark color) showed voters.
We note that if all the values in distribution are identical, then the violin plot is replaced with a single horizontal line.
...
Split in half violins no longer represent,
independent deletion of the candidates or the voters,
but correspond to simultaneous deletion of both, the candidates and the voters.
...
we provide statistical metrics of the results, i.e., mean, standard deviation, minimum and maximum.
Values are grouped by experiment parameters, in the same way as in the graphs,
...
\noindent \textbf{Summary.} \indent
First, we showed how results from Experiment 3 further confirm our conclusions from Experiment 2.
Moreover, our results provide strong basis,
to consider transforming a TVCR profile into a CR or a VR one,
if it would be required by other applications. 
