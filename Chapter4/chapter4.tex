...
check how often a profile has the TVCR restriction, compared to the other ones.   
...
we introduce additional notations to describe some special cases of the preference restrictions.
\begin{definition}
...
\begin{definition}
    ...
    if P satisfies both the VR and CR properties
    (see Definition \ref{ch3:def:c1p}, for a reminder of the COP).
\end{definition}
...
and if so we checked again if it was the TVCR variant.
We focused on the small size of elections, checking six different sizes.
Having $M$ candidates and $N$ voters, we considered $(M,N) \in \{(3,3), (4,4), (5,5), (4,6), (6,4)\}$.
...
Another observation is a decrease in a number of the VCR profiles, in favor of the not VCR ones,
...
\subsection{Experiment 2: How many profiles are TVCR, CR, VR or FCOP?}
...
\begin{enumerate}
    \item The \textit{Uniform Candidate Positions, Uniform Voter Positions (UCP/UVP)} model,
    where the points for both, the candidates and the voters are selected uniformly at random from the $[-10,10]$ interval.
    
    \item The \textit{Asymmetric Gauss Candidate Positions, Asymmetric Gauss Voter Positions (GCP/GVP)} model,
...
\begin{proposition}\label{prop:vcr-constR-vrcr}
    ...
    this profile satisfies both the VR and CR properties (it is FCOP). 
    \begin{proof}
...
\begin{figure}[H]
    ...
    Solid color corresponds to $1$ and white to $0$ in cells of the matrix. 
...
However, in certain circumstances, these blocks might have some irregularities (i.e., a 0 in sequence of 1's).
...
while maintaining general diagonal direction, they have a significant shift.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
\caption{Number of profiles with a given property, for the elections of 20 candidates and 20 voters,
...
and position candidate/voter models are represented on the x-axis. 
}
...
visualized in Figure \ref{fig:ex42-20C20V-bars} and Figure \ref{fig:ex42-40C40V-bars}.
In Figure \ref{fig:ex42-20C20V-bars} we show how many of the generated profiles have a given preference restriction.
To understand the results better, we split the collected data into groups.
We use separate subplots to distinguish the radius candidate/voter models,
and the position candidate/voter models are presented on the x-axis of each subplot.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
\caption{Number of profiles with a given property, for the elections of 40 candidates and 40 voters,
from Experiment 2. Results are grouped by the parameters used for the profile generation.
Each subplot corresponds to the selected radius candidate/voter model,
and position candidate/voter models are represented on x-axis. 
}
\label{fig:ex42-40C40V-bars}
\end{figure}

In the subsequent paragraphs we will focus mostly on the results for smaller elections from
Figure \ref{fig:ex42-20C20V-bars}, but only because the differences between the parameters are more pronounced.
\newline
...
\subsection{Experiment 3: How different is a TVCR profile?}
