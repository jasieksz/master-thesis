...
% Section 1 Control
%===============================
\section{Related Work (WIP: Inny tytuł?)}\label{ch5:sec1:overview}
...
Let us consider a problem of the CC-DV under the approval voting rule.
...
a designated winner candidate $p$, and a maximum number of voter deletions $k$.
For each candidate $c \in C$, we say that the voters who approve of $c$ form an interval: $v_{s(c)}, v_{s(c)+1}, ..., v_{f(c)}$ (the CR property).
...
a designated winner candidate $p$, and a maximum number of voter deletions $k$.
First, we discuss some concepts and observations, that will help us explain the algorithm afterwards.
...
\begin{example}\label{ex:cc-dv-vcr}
    Let us discuss an instance of the CC-DV problem. Having the VCR election from Figure \ref{fig:cc-dv-vcr},
...
In the VCR model it is possible that some candidates have their interval fully contained inside other candidate's interval. 
...
\begin{definition}\label{ch5:def:delta-notation}
    Having an instance of the CC-DV problem, with an approval election $E$, a set of candidates $C$, and the designated winner candidate $p$.
...
The internal candidate $c_I$ might not even be a dangerous candidate at all (i.e., $\Delta(c_I) \leq 0$). 
...
It means that to defeat $c_E$ we have not other choice but to delete at least $\Delta(c_I)$ voters from $\widehat{V}_I$,
...
We show how our CC-DV algorithm for the VCR elections is equivalent to the one from \cite{control-cr-how-hard-magiera-faliszewski} for the CR elections.
...
\begin{definition}
    Having a solution $S$ to the CC-DV problem, that is, a set of $k$ deleted voters.
    We define the score of this solution as a sum of removed voters' range ends: \[score(S) = \sum_{i=1}^{k} e(v_i) \]
    \textcolor{orange}{(WIP: Czy możemy zdefiniować jakiś alternatywny wynik, czy wynik musi być po prostu równy k?)}
\end{definition}
