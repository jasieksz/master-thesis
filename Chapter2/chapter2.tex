...
\begin{figure}[t!]
    \begin{subfigure}{\textwidth}
      \centering
      \includegraphics[width=\linewidth]{vcr-election-example.png}
    ...
    \begin{subfigure}{0.5\textwidth}
      \centering
      \includegraphics[width=1.1\linewidth]{vcr-election-example-matrix-unordered.png}
      \caption{Unordered approval matrix representation}
      \label{fig:vcr-example-unord-matrix}
    \end{subfigure}
    \begin{subfigure}{0.5\textwidth}
      \centering
      \includegraphics[width=1.1\linewidth]{vcr-election-example-matrix-ordered.png}
      \caption{Ordered approval matrix representation}
      \label{fig:vcr-example-ord-matirx}
    \end{subfigure}
    \caption{Three graphical representations of the same approval election from Example \ref{ex:approval-election}.
    The candidates are labeled with $C$ and the voters with $V$.
    The first one is a visualizes agents in the VCR model, i.e., every agent is represented by the point $x$ and radius $r$.
    The next two are approval matrices of the election.
    Solid color corresponds to $1$ and white to $0$ in the cells of the matrix 
    One is presented in a natural order of the agents, and in the other one, agents are ordered by their point $x$.
    }
...
\begin{example}\label{ex:approval-election}
    ...
    \end{align*}
    The reader may want to know who should be a winner given this approval profile. 
    We give answer (one of many possible) to this question in the next section in Example \ref{ex:approval-voting}.   
\end{example}
...
\begin{example}\label{ex:approval-voting}
    ...
    \end{align*}
    For a given committee size $k$, AV returns $k$ most approved candidates. 
    For example, if our desired committee size is $k=1$, then AV will return two tied candidates $c_2$ and $c_0$.
    If $k=3$, then AV will return the committee $W = \{c_0, c_2, c_4\}$.
\end{example}
...
\end{enumerate}
Typically, in literature these models are known as the candidate interval and the voter interval ones, respectively.
...
\begin{figure}[t!]
    \begin{subfigure}{0.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{cr-election-example.png}
      \caption{CR election}
    ...
    \begin{subfigure}{0.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{vr-election-example.png}
      \caption{VR election}
